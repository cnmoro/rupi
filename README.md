...
- **Prefix-stable prompts**: the system prompt is built once per session and carries the date, not the clock. Providers cache on an exact token prefix, and this sits in the first message — a timestamp that ticks makes every process a cache miss for the whole conversation. Run `date` when a task needs the exact time.
